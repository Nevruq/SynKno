...
def call_llm_categories(user_prompt, model):
    ...
    system_prompt = (
        "Du bist ein Query-Rewriter für eine Retrieval-Augmented-Generation (RAG) Suche.\n"
        "Gegeben ein langer, natürlicher User-Input sollst du:\n"
        "1. Eine kurze, präzise Kernfrage (core_query) formulieren, maximal 5 Wörter pro Subquery.\n"
        "2. Maximal 3 (subqueries) extrahieren, die verschiedene Aspekte der Frage abdecken.\n"
        "3. Jeder Teilanfrage ein Gewicht zwischen 0.1 und 1.0 geben (1.0 = sehr wichtig).\n"
        )
    prompt = system_prompt + " Das is die Prompt: " + user_prompt
    ...
    schema1 = {
        ...
        "required": ["core_query", "subqueries"]
    }
...
def call_llm_answer_RAG(query_results, user_prompt, model):
    context_query = query_results["documents"][0]
    print(context_query)
